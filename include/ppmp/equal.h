...
/**
 * @brief 判断是否定义了#define __equal_def__token(x) x 宏，未定义则留下__equal_def__token(())，没有被小括号包围故__in_matched_paren__()宏将其判定为0；如果已经定义，则只留下括号()，判定为1
 */
#define __equal_defined__(token) __in_matched_paren__(__cat__(2, __equal_def__, token)(()))
...
#define __not_equal__(token1, token2)\
		...
		(\
			__in_matched_paren__(__cat__(2, __equal_def__, token1)(__cat__(2, __equal_def__, token2))(())),\
			1\
		)
